...
{ 
	namespace code_generator
	{
		...
		//
		//-------------------------------------------------------------------------
		CLLVMCodeGen::CLLVMCodeGen(
			CErrorCodeGen & codeGeneratorErrors,
			CModuleCodeGen & namespaceCodeGenerator) :
			m_builder(std::make_shared<IRBuilderType>(getContext())),
			m_llvmModule(std::make_shared<llvm::Module>("unilang-llvm", getContext())),
		...
		//
		//-------------------------------------------------------------------------
		llvm::LLVMContext & CLLVMCodeGen::getContext() const
		{
		...
		//
		//-------------------------------------------------------------------------
		std::shared_ptr<llvm::IRBuilder<>> CLLVMCodeGen::getBuilder()
		{
			return m_builder;
		}
		//-------------------------------------------------------------------------
		//
		//-------------------------------------------------------------------------
		std::shared_ptr<llvm::Module> CLLVMCodeGen::getModule() const
		{
			return m_llvmModule;
		}
		//-------------------------------------------------------------------------
		...
		//-------------------------------------------------------------------------
		void CLLVMCodeGen::optimizeModule() const
		{
			LOG_SCOPE_DEBUG;
#ifdef _DEBUG
			...
			for(auto functionListIterator(m_llvmModule->getFunctionList().begin()); functionListIterator != m_llvmModule->getFunctionList().end(); ++functionListIterator)
			{
				llvm::FunctionPassManager OurFPM(m_llvmModule.get());
		...
		//
		//-------------------------------------------------------------------------
		void CLLVMCodeGen::verifyModule() const
		{
			std::string sError;
			llvm::raw_string_ostream rso(sError);
			if(llvm::verifyModule(*m_llvmModule.get(), &rso))
			{
		...
		//
		//-------------------------------------------------------------------------
		void CLLVMCodeGen::printModuleBytecode() const
		{
			LOG_SCOPE_DEBUG;
		...
		//
		//-------------------------------------------------------------------------
		llvm::Function * CLLVMCodeGen::getFunctionFromNameInNamespaceHierarchy( std::string const & sMangledName, std::vector<std::string> vsNamespaceHierarchy )
		{
			...
			return m_codeGeneratorErrors.getErrorFunction("Function '" + sMangledName + "' has not been decalred.");
		}
		//-------------------------------------------------------------------------
		//
		//-------------------------------------------------------------------------
		bool CLLVMCodeGen::hasFunctionFromMangledName( std::string const & sMangledName ) const
		{
		...
		//
		//-------------------------------------------------------------------------
		llvm::Function * CLLVMCodeGen::getFunctionFromMangledName( std::string const & sMangledName )
		{
			llvm::Function * pFunction (getModule()->getFunction(sMangledName));
			if (!pFunction)
			{
