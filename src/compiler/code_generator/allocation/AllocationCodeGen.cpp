...
{ 
	namespace code_generator
	{
		...
		//
		//-------------------------------------------------------------------------
		CAllocationCodeGen::CAllocationCodeGen(
		...
		//
		//-------------------------------------------------------------------------
		llvm::AllocaInst * CAllocationCodeGen::createEntryBlockAlloca(llvm::Function * const TheFunction, llvm::Type * const pType, std::string const & sVarName)
		{
		...
		//
		//-------------------------------------------------------------------------
		llvm::Value * CAllocationCodeGen::createCast(llvm::Value * pVal, llvm::Type * const pDestinationType)
		{
			if(!pVal)
			{
				return m_codeGeneratorErrors.getErrorValue("Invalid pInitVal for cast.", EErrorLevel::Internal);
			}
			if(!pDestinationType)
			{
				return m_codeGeneratorErrors.getErrorValue("Invalid pDestinationType for cast.", EErrorLevel::Internal);
			}
			...
			/*if(llvm::CastInst::isCastable(pVal->getType(), pDestinationType))
			{
				...
				if(!pVal)
				{
					return getErrorValue("Failure during cast of value of type '"+getLLVMTypeName(pInitType)+"' to type '"+ getLLVMTypeName(pDestinationType) +"'.", EErrorLevel::Internal);
				}
			}*/
				...
				if(pDestinationType->isIntegerTy())
				{
					if(!pVal->getType()->isIntegerTy())
					{
						...
						{	
							...
							if(!pVal)
							{
								return m_codeGeneratorErrors.getErrorValue("Failure during cast of value of type '"+getLLVMTypeName(pInitType)+"' to type '"+ getLLVMTypeName(pDestinationType) +"'.", EErrorLevel::Internal);
							}
						}
						else
						{
							return m_codeGeneratorErrors.getErrorValue("Can not cast value of type '"+getLLVMTypeName(pInitType)+"' to type '"+ getLLVMTypeName(pDestinationType) +"'.");
						}
					}
					...
					if(pVal->getType() != pDestinationType)
					{
						...
						if(!pVal)
						{
							...
							return m_codeGeneratorErrors.getErrorValue("Failure during cast of value of type '"+getLLVMTypeName(pInitType)+"' to type '"+ getLLVMTypeName(pDestinationType) +"'"+strIntermediateCast+".", EErrorLevel::Internal);
						}
					}
				}
