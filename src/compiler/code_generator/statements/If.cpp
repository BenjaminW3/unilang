...
{ 
	namespace code_generator
	{
		...
		//
		//-------------------------------------------------------------------------
		bool CStatementCodeGen::operator()(ast::SIfStatement const & x)
		{
			LOG_SCOPE_DEBUG;

			// generate condition value
			llvm::Value * const CondV (m_expressionCodeGenerator(x.m_expCondition));
			if (!CondV)
			{
				std::stringstream sstr;
				sstr << "Unable to generate condition: '" << x.m_expCondition << "' in SIfStatement '" << x << "'.";
			...
			{
				std::stringstream sstr;
				sstr << "Unable to get parent function for SIfStatement '" << x << "'.";
				return m_codeGeneratorErrors.ErrorBool( sstr.str() );
			}
			...
			{
				std::stringstream sstr;
				sstr << "Unable to generate condBranch in SIfStatement '" << x << "'.";
				return m_codeGeneratorErrors.ErrorBool( sstr.str() );
			}
			...
			{
				std::stringstream sstr;
				sstr << "Unable to generate then block: '" << x._thenStatementList << "' in SIfStatement '" << x << "'.";
			...
			if(!thenMergeBranch)
			{
				...
				//MergeBB->eraseFromParent();									// not attached here
				std::stringstream sstr;
				sstr << "Unable to generate thenMergeBranch in SIfStatement '" << x << "'.";
				return m_codeGeneratorErrors.ErrorBool( sstr.str() );
			}
			...
			if(x._elseOptionalStatementList.is_initialized())
			{
				...
				{
					std::stringstream sstr;
					sstr << "Unable to generate else block: '" << x._elseOptionalStatementList.get() << "' in SIfStatement '" << x << "'.";
					condBranch->eraseFromParent();
					ThenBB->eraseFromParent();
				...
				if(!elseMergeBranch)
				{
					...
					//MergeBB->eraseFromParent();								// not attached here
					std::stringstream sstr;
					sstr << "Unable to generate elseMergeBranch in SIfStatement '" << x << "'.";
					return m_codeGeneratorErrors.ErrorBool( sstr.str() );
				}
